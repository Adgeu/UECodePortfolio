...
def remove_unused(instant_delete):
    ...
    for asset in to_be_deleted:
        ...
        if instant_delete:
            ...
            if not deleted:
                unreal.log_error("Asset {} could not be deleted".format(asset_name))
                continue
        ...
        # move the assets to the trash folder
            ...
            if not moved:
                unreal.log_error("Asset {} could not be moved to Trash".format(asset_name))
                continue

            removed += 1

    output_test = "removed" if instant_delete else "moved to the Trash folder"
